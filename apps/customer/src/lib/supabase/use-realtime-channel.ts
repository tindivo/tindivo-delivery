 ...
 *
 * Versión idéntica a la de `apps/web` (deliberadamente duplicada para
 * mantener cada app independiente sin compartir hooks de Supabase entre
 * dominios distintos).
 */
 ...
 *
 * Nota: a diferencia de apps/web, no propagamos JWT en `TOKEN_REFRESHED`
 * porque el cliente es anónimo. Cuando agreguemos cuentas, restaurar el
 * `onAuthStateChange` con `realtime.setAuth(access_token)`.
 */
