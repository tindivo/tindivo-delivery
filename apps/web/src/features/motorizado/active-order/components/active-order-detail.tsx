...
export function ActiveOrderDetail({ orderId }: Props) {
  ...
  // Caso A: el pedido ya está vinculado a una dirección guardada Y tiene
  // coordenadas (en el snapshot del order O en el customer_address referenciado).
  // Esto evita re-capturar GPS en cada entrega para clientes recurrentes.
  const addressCoords =
    raw.customer_addresses?.lat != null && raw.customer_addresses?.lng != null
  const isCaseA = !!raw.customer_address_id && (persistedCoords || addressCoords)
  ...
  return (
    ...
    >
        ...
        {status === 'picked_up' && (
          <>
            ...
            <Button
              ...
              onClick={() => {
                if (isCaseA) {
                  // Caso A: usar coordenadas existentes (snapshot del order o
                  // del customer_address) como addressCapture para que cada
                  // entrega mantenga actualizadas las coordenadas en BD.
                  const useLat =
                    raw.delivery_lat ?? raw.customer_addresses?.lat
                  const useLng =
                    raw.delivery_lng ?? raw.customer_addresses?.lng
            ...
            {isCaseA && (
              <button
                type="button"
                disabled={delivered.isPending}
                onClick={() => setAddressCaptureOpen(true)}
                className="w-full inline-flex items-center justify-center gap-2 h-10 px-4 rounded-xl bg-surface-container-lowest border border-outline-variant/40 text-sm font-semibold text-on-surface-variant active:scale-[0.98] transition-transform"
              >
      ...
      <AddressCaptureModal
        ...
        initialLat={raw.customer_addresses?.lat ?? undefined}
        initialLng={raw.customer_addresses?.lng ?? undefined}
        initialReference={raw.delivery_reference}
        initialCustomerName={raw.client_name ?? undefined}
