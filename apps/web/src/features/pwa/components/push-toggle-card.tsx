...
import { type SubscribeFailReason, usePushSubscription } from '../hooks/use-push-subscription'

/**
 * Mapea la causa específica del fallo a un mensaje accionable. Cada
 * `reason` viene del hook (ver `SubscribeFailReason`). Mantener este
 * mapping sincronizado con cualquier nuevo punto de fallo.
 */
function messageFromSubscribeReason(reason: SubscribeFailReason): string {
...
export function PushToggleCard() {
  ...
  async function handleToggle() {
    ...
    if (!result.ok) setErrorMsg(messageFromSubscribeReason(result.reason))
  }
